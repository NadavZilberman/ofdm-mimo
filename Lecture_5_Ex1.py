...
def main():
    ...
    for i, [M, N] in enumerate(ant_pairs):
        ...
        for snr in snr_array:
            ...
            y = np.matmul(H_tild, x) + rho * noise
        ...
        plt.plot(snr_array_dB, SER_array_SM[i], label=f"SM with ML and SVD precoding {M}Tx X {N}Rx")
    ...
    for i, [M, N] in enumerate(ant_pairs):
        ...
        plt.plot(snr_array_dB, SER_array_ZF[i], label=f"ZF with SVD precoding {M}Tx X {N}Rx")

    plt.legend()
    plt.grid(which='both', axis='both')
    plt.xticks(snr_array_dB)
    plt.yscale('log')
    plt.title(f'SER vs SNR - SM with ML detection vs ZF - SVD precoding (2Tx X 2Rx) and (2Tx X 4Rx)\n{REP} repetitions')
    plt.ylabel('SER')
    plt.xlabel('SNR[dB]')
    plt.show()
